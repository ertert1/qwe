...
public class calculator extends JFrame {
    ...
    public calculator() {
        ...
        add(buttonResult, BorderLayout.SOUTH); //Добавляем кнопку суммы и указываем ее расположение
        JPanel radioPanel = new JPanel(new GridLayout(5, 1)); // Определяем расположение элементов
        radioPanel.setBackground(new Color(207, 207, 207)); //Устанавливаем цвет панелей
        radioPanel.setLayout((LayoutManager) new BoxLayout(radioPanel, BoxLayout.Y_AXIS)); // Указываем расположение строк
        setBounds(300, 300, 300, 300); //Определяем размер окна   
        ...
        calculate.setBackground(new Color(255, 128, 0)); //Задаем цвет кнопки
...
public float Result(float sum,float stavka,float month) { //Задаем переменныеи метод
